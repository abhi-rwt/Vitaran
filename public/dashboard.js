...
// Update DOM elements based on requirements
...
if(earningsEl) earningsEl.innerText = ₹${stats.earnings};
...
// 🔥 LOCK LOGIC FIX: Check against lastDelivered state
const locked = (lastDelivered === "high" && isHigh);
tr.innerHTML = <td> <img src="${getLogo(o.platform)}"> ${o.platform} </td> <td>#VT${o.id}</td> <td><span class="tag">COD</span></td> <td>₹${o.amount}</td> <td>${o.km} KM</td> <td class="${isHigh ? 'green':''}">₹${o.profit}</td> <td> <button class="btn accept ${locked?'disabled':''}" ${locked?'disabled':''}> ${locked?'Locked':'Accept'} </button> </td>;
...
modal.innerHTML = <div class="modal-box"> <div id="camBox" style="width:100%; height:250px; background:#000; border-radius:12px; overflow:hidden;"></div> <h3 style="margin-top:10px;">Capture Photo</h3> <button class="btn primary" style="width:100%; margin-top:10px;">Capture</button> </div>;
